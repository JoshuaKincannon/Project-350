...
public class gameLauncher implements ActionListener {
     ...
     * details
     */
    public gameLauncher(String filename){
        logPanel = new JPanel();
        logFrame = new JFrame();
...
//        launchGames();
        ...
        if(!hasUserName && !results.getText().equals("User Already Exists") &&
                createPass.equals(verifyPass) && !createUsers.isEmpty() && !verifyPass.isEmpty()){
            try {
                FileWriter writer = new FileWriter(passwords);
                writer.write(createUsers + "," + createPass + "\n");
    ...
    public JTextField getCreateUserText() {
        return createUserText;
    }

    public JTextField getCreateUserPassword() {
        return createUserPassword;
    }

    public JTextField getVerify(){
        return verify;
    }
    ...
    public static void main (String[]args){
        new gameLauncher("passwords.txt");
    }
}